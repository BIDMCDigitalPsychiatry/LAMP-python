...
def label_gps_points(sensor_data):
    ...
    for (_, point1), (_, point2) in zip(sensor_data['lamp.gps'].iterrows(), sensor_data['lamp.gps'].shift(-1).iterrows()):
        ...
        if point1['timestamp'] == labeled_data.loc[labeled_data.index[-1], 'timestamp']:
            ...
            speed =  dist / ((datetime.utcfromtimestamp(point2['timestamp'] / 1000) - datetime.utcfromtimestamp(point1['timestamp'] / 1000)).seconds / 3600)
...
def get_trips(df, state=False, inclusive=True):
    ...
    for tup in idx_list:
        ...
        trip = (df['timestamp'][tup[0]], df['timestamp'][tup[1]],dist)
...
def all(sensor_data, dates, resolution):
    ...
    interval_range = pd.interval_range(labeled_data.loc[labeled_data.index[0], 'timestamp'], 
                                       labeled_data.loc[labeled_data.index[-1], 'timestamp'], 
                                       freq=resolution)
